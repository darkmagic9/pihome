...
if($what=="device"){
	...
	if($opp=="insert"){
		...
		mysqli_query(dbconnect(), $sql_room_update);
	}elseif($opp=="delete"){
		dbconnect();
		$sql_delete_room = "DELETE FROM pi_rooms WHERE id = ".$wid;
		mysqli_query(dbconnect(), $sql_delete_room);
		echo "del room ".$wid;
	}
